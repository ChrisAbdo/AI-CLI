...
async function deleteApiKeyFile() {
  if (await Bun.file(apiKeyFilePath).exists()) {
    ...
    console.log(
      chalk.green(
        "API key reset successfully. Restart the script with a new instruction to enter a new API key."
      )
    );
    console.log(
      chalk.yellow("Note: The API key file was deleted from " + apiKeyFilePath)
    );
    process.exit(0);
  } else {
    console.log(chalk.yellow("No API key to reset."));
  }
}
...
async function getOrRequestApiKey(): Promise<string> {
 ...
 |_|_|  \\___/|_|  \\__, |\\___|\\__|
  ...
  console.log(
    chalk.whiteBright(
      "Welcome to the IForget CLI! Run `npx iforget help` for more information.\n"
    )
  );
  console.log(
...
if (process.argv.includes("reset")) {
  // deleteApiKeyFile().then(() => process.exit(0));
  ...
  if (await Bun.file(apiKeyFilePath).exists()) {
    ...
    console.log(
      chalk.yellow(
        "No API key to reset. Try running the script with a new instruction."
      )
    );
    process.exit(0);
  }
...
(async () => {
  ...
  if (!userInstruction) {
    console.error(
      chalk.red(
        "Please provide a command instruction in this format: " +
          "npx iforget " +
          '"your instruction here"'
      )
    );
    console.error(chalk.yellow('For more information, run "npx iforget help"'));
    process.exit(1);
  }
